...
def productPredict(input_path):
    ...
    '''
    image_names = []
    DEMO_IMAGE_DIR="/home/FCIS/demo"
    demo_imnames=os.listdir(DEMO_IMAGE_DIR)
    for i in demo_imnames:
        image_names.append(i)
    '''

    data = []
   
    assert os.path.exists(input_path), ('%s does not exist'.format(input_path))
    im = cv2.imread(input_path, cv2.IMREAD_COLOR | cv2.IMREAD_IGNORE_ORIENTATION)
    ...
    for i in xrange(len(dets)):
        keep = np.where(dets[i][:,-1]>0.8)
        dets[i] = dets[i][keep]
        masks[i] = masks[i][keep]

        if len(dets[i]) > 0:
            className.append([dets[i],classes[i]])
    
    #print(dets[0])

    im = cv2.imread(input_path)
    im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
    img,cutImages,masks = show_masks(im, dets, masks, classes, config)

    return img,className,cutImages,masks
